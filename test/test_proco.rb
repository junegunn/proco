...
describe Proco do
  it "can be created with method chaining" do
    proco = Proco.interval(1).threads(4).queues(4).queue_size(100).batch(true).new
    opts = proco.options
  ...
  describe "in batch-mode with multiple queues" do
    before do
      @proco = Proco.queue_size(10).queues(2).threads(1).batch(true).new
    end
...
#       cnt += items.inject(:+)
#     end
#     1000.times { |i| @proco.submit i, i, i }
#     assert_equal 1000.times.inject(:+) * 3, cnt
